...
std::string ConnectionInfoReader::read() const
{
  ...
  auto          config = fmt::format("--SERVER={}:{}", ip, port);
  ...
  PRECICE_CHECK(rc == MEMCACHED_SUCCESS, "Failed to read key {} from memcached server {}:{}. {}", key, ip, port, memcached_strerror(memc, rc));
